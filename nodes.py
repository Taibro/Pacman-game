...
class Node(object):
    def __init__(self, x, y):
        ...
        self.access = {UP:[PACMAN, BLINKY, INKY, CLYDE, PINKY, FRUIT],
                       ...
                       LEFT:[PACMAN, BLINKY, INKY, CLYDE, PINKY, FRUIT],
                       RIGHT:[PACMAN, BLINKY, INKY, CLYDE, PINKY, FRUIT]}
...
class NodeGroup(object):
    ...
    def connectVertically(self, data, xoffset = 0, yoff_set = 0):
        ...
        for col in list(range(dataT.shape[0])):
            ...
            for row in list(range(dataT.shape[1])):
                if dataT[col][row] in self.nodeSymbols:
                    if key is None:
                        key = self.constructKey(col + xoffset, row + yoff_set)
                    else:
                        otherKey = self.constructKey(col + xoffset, row + yoff_set)
